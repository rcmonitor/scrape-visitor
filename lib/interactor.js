...
Interactor.prototype.on(Interactor.availableEvents.actionBegin, function(){
	...
	this.request(this.requestOptions, function(error, oIncomingMessage, strPage){
		if(error){
			...
			context.logResponse(oIncomingMessage, strPage);

			context.inspector.inspect(oIncomingMessage, strPage);
...
Interactor.prototype.logResponse = function(oIncomingMessage, strPage){
