...
function Strategy(oConfig, oInteractor){
	...
	if(this.validateConfig(oConfig)){
		...
		this.extend(this.commonParameters, this.config.common);
...
Strategy.prototype.load = function(){
	...
	if(oConfig){
		...
		if(this.config.inspector[this.interactor.currentActionName]){
			this.interactor.inspector.assertions
			...
			this.interactor.fileLogger.log('config contains no assertions for action "'
...
Strategy.prototype.addError = function(error){
	...
	if(hpg.getType(error) == 'array'){
		this.interactor.errors = this.interactor.errors.concat(error);
	}else{
		this.interactor.errors.push(error);
	}
};
